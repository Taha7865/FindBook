...
Description-only searches request work fields: title, authors, subjects, first publication year, cover, and reading-list count. They omit nested edition fields because requesting those fields can exclude works that match through their subjects. These results have no edition list unless a focused title, author, or edition search supplied one.

Title and author searches request `editions.publish_date` and display the first nonblank date supplied for each returned edition. This needs no additional HTTP request and never borrows a date from another edition or the work's publication-date list. Missing dates remain unavailable; catalog date text is preserved.
...
Docker Compose builds and starts the API and frontend. API health and live searches through the frontend have been verified with a runtime key. A future function-calling version can reuse `OpenLibraryApiClient`; the current implementation uses explicit calls.
