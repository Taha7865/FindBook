...
All 141 unit tests pass, including 44 service cases. The release publish also passes. The accepted-only helper and both author-fallback paths were verified with injected client doubles; live Gemini accuracy was not re-evaluated for this change. Earlier release-published HTTP checks used local upstream fixtures for the complete call sequence, grouped metadata, invalid selections, empty results, missing credentials, and 502/503/504 responses. The published API also recovered from simulated Gemini 503 and Open Library 429 responses; persistent Gemini failures stopped after three attempts and returned HTTP 503.
