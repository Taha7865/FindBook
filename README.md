...
Primary-author verification and ranking remain unresolved; the response uses `authors[]`. Edition contributions are passed through as catalog text. The search endpoint supplies one preferred edition per work, and only the configured number of edition records are checked. A missing or uncertain date leaves the requested edition unverified. The edition list is not exhaustive. Clear-winner selection, distinguishing adaptations from original works, explanation accuracy, and fallback after Gemini rejects a nonempty catalog result still need work.
