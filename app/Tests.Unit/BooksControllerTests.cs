...
public sealed class BooksControllerTests
{
    ...
    [Theory]
    [InlineData(AiFailure.BadResponse, 502)]
    [InlineData(AiFailure.Unavailable, 503)]
    [InlineData(AiFailure.NotConfigured, 503)]
    [InlineData(AiFailure.Timeout, 504)]
    public async Task Ai_failures_return_problem_details(AiFailure failure, int status)
    {
    ...
    private sealed class StubGemini(AiFailure? failure = null) : IGeminiApiClient
    {
        public Task<BookSearchTerms> ExtractSearchTermsAsync(string userQuery, CancellationToken cancellationToken)
            => failure is { } error ? throw new AiException(error) : Task.FromResult(new BookSearchTerms("Example", null, [], null, []));
